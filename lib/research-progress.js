...
export function getResearchProjectGoalStage(projectType) {
  const stages = getResearchProjectStages(projectType);
  if (!stages.length) return "revision_or_close";
  const goalStage = stages[stages.length - 1];
  if (goalStage !== "revision_or_close") return goalStage;
  return stages[stages.length - 2] || goalStage;
}
