...
export async function getCommunityFeed() {
  ...
  const [{ data: profileRows }, { data: postRows }, { data: tagRows }] = await Promise.all([
    ...
    supabase
      .from("posts")
      .select(
        "id, slug, title, excerpt, published, visibility, scheduled_for, published_at, updated_at, tags, cover_image_url, profiles(id, username, display_name, avatar_url, account_status, discoverable)"
      )
      .eq("published", true)
...
export async function getExploreData({ query = "", sort = "newest" } = {}) {
  ...
  const sortedProfiles =
    sort === "popular"
      ...
      : [...profiles].sort((left, right) => compareDates(right.updated_at, left.updated_at));
...
export async function getPostByUsernameAndSlug(username, slug) {
  ...
  const { data } = await supabase
    .from("posts")
    .select(
      "id, slug, title, excerpt, body, published, tags, media_items, cover_image_url, allow_comments, published_at, updated_at, visibility, scheduled_for, author_id, profiles!inner(id, username, display_name, avatar_url, account_status, discoverable)"
    )
    .eq("author_id", profile.id)
    .eq("slug", slug)
    .eq("published", true)
...
function isDiscoverableAuthor(profile) {
  return isVisibleAuthor(profile) && profile.discoverable !== false;
}
