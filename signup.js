...
document
  .querySelectorAll(".mobile-nav a, .desktop-nav a")
  ...
  document
    ...
    .forEach((node) => {
      node.addEventListener("mouseenter", () =>
        cursorOrbit.classList.add("hovering")
      );
      node.addEventListener("mouseleave", () =>
        cursorOrbit.classList.remove("hovering")
      );
    });
}

/* ===== Signup form handling ===== */
document.querySelectorAll(".signup-inline-form").forEach((form) => {
  form.addEventListener("submit", (event) => {
    ...
    const button = form.querySelector(".signup-cta");
    ...
    setTimeout(() => {
      ...
      alert(
        `@${username} で登録を開始します（デモ）\n\n実際の登録フローに遷移します。`
      );
    }, 800);
  });
});

/* ===== Username input: strip invalid chars ===== */
document
  .querySelectorAll('.signup-input-group input[name="username"]')
  .forEach((input) => {
...
/* ===== Network background (reused from main) ===== */
function initNetworkBackground(canvas) {
  ...
  function createNodes() {
    ...
    for (let row = 0; row <= rows; row += 1) {
      for (let col = 0; col <= cols; col += 1) {
        ...
        if (col < cols)
          state.edges.push({ a: current, b: indexOf(row, col + 1) });
        if (row < rows)
          state.edges.push({ a: current, b: indexOf(row + 1, col) });
  ...
  function draw(time) {
    ...
    const dt = lastFrameTime
      ? Math.min((time - lastFrameTime) * 0.001, 0.05)
      : 0.016;
    lastFrameTime = time;

    const nextHeight = getDocumentHeight();
    if (Math.abs(nextHeight - state.height) > 8) resize();
    ...
    for (const node of state.nodes) {
      ...
      for (const impulse of state.impulses) {
        ...
        const force =
          (1 - dist / impulse.radius) * impulse.strength * impulse.life;
    ...
    for (const spark of state.sparks) {
      ...
      const zigX =
        trailX +
        (x - trailX) * 0.56 +
        nx * spark.size * 0.92 * spark.direction;
      const zigY =
        trailY +
        (y - trailY) * 0.56 +
        ny * spark.size * 0.92 * spark.direction;
      const boltColor =
        spark.color === "red"
      ...
      if (spark.flash > 0) {
        ...
        ctx.beginPath();
        ctx.moveTo(
          start.x - spark.size * 2.8,
          start.y - spark.size * 0.9
        );
        ctx.lineTo(
          start.x + spark.size * 2.8,
          start.y + spark.size * 0.9
        ...
        ctx.moveTo(
          start.x - spark.size * 0.9,
          start.y + spark.size * 2.8
        );
        ctx.lineTo(
          start.x + spark.size * 0.9,
          start.y - spark.size * 2.8
  ...
  function spawnSparksNear(x, y, count, strength = 1) {
    ...
    for (let i = 0; i < state.edges.length; i += 1) {
      ...
      if (dist < 180) candidates.push({ edgeIndex: i, dist });
    }

    candidates.sort((lhs, rhs) => lhs.dist - rhs.dist);
    const selected = candidates.slice(
      0,
      Math.min(candidates.length, lowPowerDevice ? count : count + 2)
    );
    ...
    if (state.sparks.length > (lowPowerDevice ? 64 : 140)) {
      state.sparks.splice(
        0,
        state.sparks.length - (lowPowerDevice ? 64 : 140)
      );
    }
  }
...
function initHeroShader(canvas) {
  ...
  const fragmentSource = `
    ...
    float neuralLines(vec2 p, vec2 flow, float t) {
      ...
      return max(smoothstep(0.09, 0.0, lineA), smoothstep(0.08, 0.0, lineB));
    }
  ...
  function render(time) {
    ...
    gl.uniform1f(
      scrollLocation,
      Math.min(
        window.scrollY / Math.max(entranceSection.offsetHeight, 1),
        1.3
      )
    );
  ...
  function onPointerMove(clientX, clientY) {
    ...
    if (inside) {
      targetPointerVelocity = Math.min(
        1.0,
        targetPointerVelocity + Math.sqrt(dx * dx + dy * dy) * 10.0
      );
    }
    targetMouse.x = nextX;
    targetMouse.y = nextY;
  ...
  function triggerImpulse(clientX, clientY, strength = 0.9) {
    ...
    const inside =
      ...
      clientY <= rect.bottom;
    if (!inside) return;
    pulseOrigin.x = Math.min(
      1,
      Math.max(0, (clientX - rect.left) / rect.width)
    );
    pulseOrigin.y = Math.min(
      1,
      Math.max(0, (clientY - rect.top) / rect.height)
    );
    ...
    targetPointerVelocity = Math.min(
      1.0,
      targetPointerVelocity + strength * 0.7
    );
  ...
  window.addEventListener(
    ...
    () => {
      ...
      targetScrollVelocity = Math.min(
        1.0,
        targetScrollVelocity + delta / window.innerHeight
      );
      requestRender();
    },
    { passive: true }
