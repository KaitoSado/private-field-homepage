...
- 初版では DB 同期よりも学習ループの密度を優先し、進捗は `localStorage` に置く
- 感情設計は強刺激ではなく、不安を下げる説明と軽い達成感を優先する

### 14. 英語 app の復習は可変 ease ではなく固定段階で回す

- `/apps/english` の復習は、ユーザーが理解しやすい `学習直後 -> 1日後 -> 3日後 -> 8日後 -> 30日後` の固定 5 段階で扱う
- 間違えたときは 1 段階戻し、初段で間違え続ける語は短い間隔に留める
- 日時つきの生ログは内部保存しつつ、UI では「間違えた単語一覧」を主に見せて認知負荷を上げすぎない

### 15. 英語 app の語族は表示統合から始める

- `/apps/english` は当面、カード単位の正誤記録を維持したまま、同じ `family` の語を派生語として同時表示する
- `family` の機械生成だけで 1900 見出し語へ畳むと誤グループ化や分離漏れがあるため、完全な 1900 グループ化は原本見出し語IDの整備後に行う
- 後続のランダム表示、主語と派生語の入れ替え、品詞別プレイは `family` と `pos` を使って追加する
