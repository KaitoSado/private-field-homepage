...
- `public/textures/new-commune/` に生成PNG素材を置き、Apps / signature / Research Progress / 深夜徘徊 / GenerativeArtWithMath で装飾レイヤーとして使う。単一の隅置きではなく複数の紙片をページ内に分散させ、色と存在感を少し強める。装飾は `pointer-events: none` で機能面を塞がず、モバイルでは小さく薄く出す。反映安定のため layout から `/textures/new-commune/site-textures-20260427b.css` も読む
- 触る際は:
  - どの面を変えるかを明確にする
  - 無関係なセクションをまとめて触らない
