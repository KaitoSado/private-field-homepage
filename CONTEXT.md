...
- `public/textures/new-commune/` に生成PNG素材を置き、Apps / `/me` / signature / Research Progress / 深夜徘徊 / GenerativeArtWithMath で装飾レイヤーとして使う。`archteia-site-assets/` には横長の実物断片素材16枚を置き、アプリ棚、プロフィール、CURRENT、WORKS、Research、Links に振り分ける。装飾は `pointer-events: none` で機能面を塞がず、モバイルでは小さく薄く出す。反映安定のため layout から `/textures/new-commune/site-textures-20260427c.css` も読む
- 触る際は:
  - どの面を変えるかを明確にする
  - 無関係なセクションをまとめて触らない
