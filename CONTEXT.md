...
- `public/textures/new-commune/` に生成PNG素材を置き、Apps / signature / Research Progress / 深夜徘徊 / GenerativeArtWithMath で薄い装飾レイヤーとして使う。装飾は `pointer-events: none` で機能面を塞がず、モバイルでは小さく薄く出す。反映安定のため layout から `/textures/new-commune/site-textures-20260427b.css` も読む
- 触る際は:
  - どの面を変えるかを明確にする
  - 無関係なセクションをまとめて触らない
