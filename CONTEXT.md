...
- Apps
  ...
  - 英語コンテンツ `/apps/english`
    ...
    - 正誤ログは各単語ごとに日時つきで `localStorage` に保持し、復習は `当日 -> 3日後 -> 7日後 -> 14日後 -> 30日後` の固定 5 段階で回す
    - 正解すると次ステージへ進み、間違えると現在ステージに留まる
    - 出題キューは復習期限の来た間違い語を優先し、セッションごとにシャッフルする
    - 品詞フィルタで `名詞`, `動詞`, `形容詞`, `副詞`, `前置詞`, `句` に絞れる
